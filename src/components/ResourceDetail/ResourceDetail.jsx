...
export const ResourceDetail = ({ resource }) => {
  const getTags = () => {
    ...
    if (properties?.sliding_scale) {
      tags.push({ label: "Sliding scale", color: "#B8860B" });
    }

    if (properties?.families_welcome) {
      tags.push({ label: "Families welcome", color: "#2E8B57" });
    }
        ...
        <Box>
          {getTags().map((tag, index) => (
            <Chip
              key={index}
              label={tag.label}
              sx={{ bgcolor: tag.color, color: "#fff", fontWeight: "bold", marginRight: "1em" }} // Dark background and white text
            />
          ))}
        </Box>
      </Box>
